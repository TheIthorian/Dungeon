from constants import POSITIVE_EVENTS, NEGATIVE_EVENTS, ROOM_SPECIAL_EVENTS, EXPLORE_SPECIAL_EVENTS, REST_SPECIAL_EVENTS
from database import PlayerCharacter, PlayerInventory, player_character, player_inventory, char_print, char_input
        ...
        def get_effect(player: PlayerCharacter = player_character):
            if effect == "ambush_effect":
                ...
                if player_response == 'interrupt':
                    nested_response = char_input("\nYou smell a liar! What will you do? (Accuse/Let Continue): ", lower=True)
...
class ExploreSpecialEvents:
    ...
    def run_event(self):
        ...
        def get_effect(player: PlayerCharacter = player_character, inventory: PlayerInventory = player_inventory):
    ...
    def run_event(self):
        ...
        def get_effect(player: PlayerCharacter = player_character, inventory: PlayerInventory = player_inventory):
            if effect == "visitor_effect":
                player_response = char_input("Do you take the apple? (Y/N): ", lower=True)
                if player_response == 'y':
                    if randint(0, 1):
                ...
                if player_response == 'y':
                    if randint(0, 1):
                        nested_response = char_input("\nYou swat the light. When you look at your hand, you see a crushed "
                                                "pixie. What do you do? (Eat/Smear): ", lower=True)
